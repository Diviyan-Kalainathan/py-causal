    ...
    def __init__(self, tetradGraph, dfs, dataType=0, numCategoriesToDiscretize=4, rule='R1', score='andersonDarling', alpha=0.01, epsilon=1.0, zeta=0.0, orientStrongerDirection=False, r2Orient2Cycles=True, edgeCorrected=False, selfLoopStrength=1.0):
        ...
        for idx in range(len(dfs)):
            ...
            # Mixed
                ...
                tetradData = pycausal.loadMixedData(
                    df, numCategoriesToDiscretize)
            datasets.add(tetradData)

        lofs2 = javabridge.JClassWrapper(
            'edu.cmu.tetrad.search.Lofs2')(tetradGraph, datasets)
...
class fofc():
    ...
    def __init__(self, df, testType='TETRAD_WISHART', fofcAlgorithm='GAP', alpha=0.01, numBootstrap=-1, ensembleMethod='Highest'):
        ...
        if numBootstrap < 1:
            ...
            fofc = javabridge.JClassWrapper('edu.cmu.tetrad.search.FindOneFactorClusters')(
                tetradData, testType, fofcAlgorithm, alpha)
        else:
            ...
            fofc = javabridge.JClassWrapper('edu.pitt.dbmi.algo.bootstrap.GeneralBootstrapTest')(
...
class dm():
    ...
    def __init__(self, df, inputs, outputs, trueInputs, useGES=True, alphaPC=0.05, alphaSober=0.05, gesDiscount=10, verbose=False, minDiscount=4):
        ...
        self.nodes = pycausal.extractTetradGraphNodes(
            self.tetradGraph, orig_columns, new_columns)
        self.edges = pycausal.extractTetradGraphEdges(
            self.tetradGraph, orig_columns, new_columns)
        self.graph = pycausal.generatePyDotGraph(self.nodes, self.edges)
...
class imagesBDeu():
    ...
    def __init__(self, dfs, structurePrior=1.0, samplePrior=1.0, maxDegree=3, faithfulnessAssumed=True, verbose=False, priorKnowledge=None, numBootstrap=-1, ensembleMethod='Highest'):
        ...
        if numBootstrap < 1:
            score = javabridge.JClassWrapper(
                'edu.cmu.tetrad.search.BdeuScoreImages')(datasets)
            score.setStructurePrior(structurePrior)
            score.setSamplePrior(samplePrior)

            fges = javabridge.JClassWrapper(
                'edu.cmu.tetrad.search.Fges')(score)
            fges.setMaxDegree(maxDegree)
            fges.setNumPatternsToStore(0)
            fges.setFaithfulnessAssumed(faithfulnessAssumed)
        else:
            algorithm = javabridge.JClassWrapper(
                'edu.cmu.tetrad.algcomparison.algorithm.multi.ImagesBDeu')()
            ...
            fges = javabridge.JClassWrapper('edu.pitt.dbmi.algo.bootstrap.GeneralBootstrapTest')(
                datasets, algorithm, numBootstrap)
            edgeEnsemble = javabridge.get_static_field('edu/pitt/dbmi/algo/bootstrap/BootstrapEdgeEnsemble',
                                                       ensembleMethod,
                                                       'Ledu/pitt/dbmi/algo/bootstrap/BootstrapEdgeEnsemble;')
...
class imagesSemBic():
    ...
    def __init__(self, dfs, penaltyDiscount=4, maxDegree=3, faithfulnessAssumed=True, verbose=False, priorKnowledge=None, numBootstrap=-1, ensembleMethod='Highest'):
        ...
        if numBootstrap < 1:
            score = javabridge.JClassWrapper(
                'edu.cmu.tetrad.search.SemBicScoreImages')(datasets)
            ...
            algorithm = javabridge.JClassWrapper(
                'edu.cmu.tetrad.algcomparison.algorithm.multi.ImagesSemBic')()
...
class fas():
    ...
    def __init__(self, df, dataType=0, numCategoriesToDiscretize=4, depth=3, alpha=0.05, verbose=False, priorKnowledge=None, numBootstrap=-1, ensembleMethod='Highest'):
        ...
        if numBootstrap < 1:
            fas = javabridge.JClassWrapper(
                'edu.cmu.tetrad.search.Fas')(indTest)
            fas.setDepth(depth)
        else:
            algorithm = javabridge.JClassWrapper(
                'edu.cmu.tetrad.algcomparison.algorithm.oracle.pattern.FAS')(indTest)
            ...
            fas = javabridge.JClassWrapper('edu.pitt.dbmi.algo.bootstrap.GeneralBootstrapTest')(
                tetradData, algorithm, numBootstrap)
            edgeEnsemble = javabridge.get_static_field('edu/pitt/dbmi/algo/bootstrap/BootstrapEdgeEnsemble',
                                                       ensembleMethod,
                                                       'Ledu/pitt/dbmi/algo/bootstrap/BootstrapEdgeEnsemble;')
...
class fgesMixed():
    ...
    def __init__(self, df, numCategoriesToDiscretize=4, penaltyDiscount=4, structurePrior=1.0, maxDegree=3, faithfulnessAssumed=True, verbose=False, priorKnowledge=None, numBootstrap=-1, ensembleMethod='Highest'):
        ...
        if numBootstrap < 1:
            score = javabridge.JClassWrapper('edu.cmu.tetrad.search.ConditionalGaussianScore')(
                tetradData, structurePrior, False)
            # set to 2 if variable# <= 50 otherwise set it to 4
            score.setPenaltyDiscount(penaltyDiscount)
            ...
            algorithm = javabridge.JClassWrapper(
                'edu.cmu.tetrad.algcomparison.algorithm.oracle.pattern.Fges')(score)
...
class fgesDiscrete():
    ...
    def __init__(self, df, structurePrior=1.0, samplePrior=1.0, maxDegree=3, faithfulnessAssumed=True, verbose=False, priorKnowledge=None, numBootstrap=-1, ensembleMethod='Highest'):
        ...
        if numBootstrap < 1:
            score = javabridge.JClassWrapper(
                'edu.cmu.tetrad.search.BDeuScore')(tetradData)
            score.setStructurePrior(structurePrior)
            score.setSamplePrior(samplePrior)
...
class fges():
    ...
    def __init__(self, df, penaltyDiscount=4, maxDegree=3, faithfulnessAssumed=True, verbose=False, priorKnowledge=None, numBootstrap=-1, ensembleMethod='Highest'):
        ...
        if numBootstrap < 1:
            ...
            score = javabridge.JClassWrapper(
                'edu.cmu.tetrad.search.SemBicScore')(tetradData)
            # set to 2 if variable# <= 50 otherwise set it to 4
            score.setPenaltyDiscount(penaltyDiscount)
...
class fci():
    ...
    def __init__(self, df, dataType=0, numCategoriesToDiscretize=4, depth=3, alpha=0.05, completeRuleSetUsed=False, verbose=False, priorKnowledge=None, numBootstrap=-1, ensembleMethod='Highest'):
        ...
        if numBootstrap < 1:
            fci = javabridge.JClassWrapper(
                'edu.cmu.tetrad.search.Fci')(indTest)
            fci.setDepth(depth)
            fci.setCompleteRuleSetUsed(completeRuleSetUsed)
        else:
            algorithm = javabridge.JClassWrapper(
                'edu.cmu.tetrad.algcomparison.algorithm.oracle.pag.Fci')(indTest)
            ...
            fci = javabridge.JClassWrapper('edu.pitt.dbmi.algo.bootstrap.GeneralBootstrapTest')(
                tetradData, algorithm, numBootstrap)
            edgeEnsemble = javabridge.get_static_field('edu/pitt/dbmi/algo/bootstrap/BootstrapEdgeEnsemble',
                                                       ensembleMethod,
                                                       'Ledu/pitt/dbmi/algo/bootstrap/BootstrapEdgeEnsemble;')
...
class cfci():
    ...
    def __init__(self, df, dataType=0, numCategoriesToDiscretize=4, depth=3, alpha=0.05, verbose=False, priorKnowledge=None, numBootstrap=-1, ensembleMethod='Highest'):
        ...
        if numBootstrap < 1:
            cfci = javabridge.JClassWrapper(
                'edu.cmu.tetrad.search.Cfci')(indTest)
            cfci.setDepth(depth)
        else:
            algorithm = javabridge.JClassWrapper(
                'edu.cmu.tetrad.algcomparison.algorithm.oracle.pag.Cfci')(indTest)
            ...
            cfci = javabridge.JClassWrapper('edu.pitt.dbmi.algo.bootstrap.GeneralBootstrapTest')(
                tetradData, algorithm, numBootstrap)
            edgeEnsemble = javabridge.get_static_field('edu/pitt/dbmi/algo/bootstrap/BootstrapEdgeEnsemble',
                                                       ensembleMethod,
                                                       'Ledu/pitt/dbmi/algo/bootstrap/BootstrapEdgeEnsemble;')
...
class rfciMixed():
    ...
    def __init__(self, df, numCategoriesToDiscretize=4, depth=3, maxPathLength=-1, alpha=0.05, completeRuleSetUsed=False, verbose=False, priorKnowledge=None, numBootstrap=-1, ensembleMethod='Highest'):
        ...
        if numBootstrap < 1:
            ...
            rfci = javabridge.JClassWrapper(
                'edu.cmu.tetrad.search.Rfci')(indTest)
            rfci.setDepth(depth)
            ...
            indTest = javabridge.JClassWrapper(
                'edu.cmu.tetrad.algcomparison.independence.ConditionalGaussianLRT')()
            algorithm = javabridge.JClassWrapper(
                'edu.cmu.tetrad.algcomparison.algorithm.oracle.pag.Rfci')(indTest)
            ...
            rfci = javabridge.JClassWrapper('edu.pitt.dbmi.algo.bootstrap.GeneralBootstrapTest')(
                tetradData, algorithm, numBootstrap)
            edgeEnsemble = javabridge.get_static_field('edu/pitt/dbmi/algo/bootstrap/BootstrapEdgeEnsemble',
                                                       ensembleMethod,
                                                       'Ledu/pitt/dbmi/algo/bootstrap/BootstrapEdgeEnsemble;')
...
class gfciMixed():
    ...
    def __init__(self, df, numCategoriesToDiscretize=4, penaltyDiscount=4, structurePrior=1.0, maxDegree=3, maxPathLength=-1, alpha=0.05, completeRuleSetUsed=False, faithfulnessAssumed=True, verbose=False, priorKnowledge=None, numBootstrap=-1, ensembleMethod='Highest'):
        ...
        if numBootstrap < 1:
            ...
            gfci = javabridge.JClassWrapper(
                'edu.cmu.tetrad.search.GFci')(indTest, score)
            ...
            score = javabridge.JClassWrapper(
                'edu.cmu.tetrad.algcomparison.score.ConditionalGaussianBicScore')()
            algorithm = javabridge.JClassWrapper(
                'edu.cmu.tetrad.algcomparison.algorithm.oracle.pag.Gfci')(indTest, score)
            ...
            gfci = javabridge.JClassWrapper('edu.pitt.dbmi.algo.bootstrap.GeneralBootstrapTest')(
                tetradData, algorithm, numBootstrap)
            edgeEnsemble = javabridge.get_static_field('edu/pitt/dbmi/algo/bootstrap/BootstrapEdgeEnsemble',
                                                       ensembleMethod,
                                                       'Ledu/pitt/dbmi/algo/bootstrap/BootstrapEdgeEnsemble;')
...
class ccd():
    ...
    def __init__(self, df, dataType=0, numCategoriesToDiscretize=4, depth=3, alpha=0.05, priorKnowledge=None, numBootstrap=-1, ensembleMethod='Highest'):
        ...
        if numBootstrap < 1:
            ccd = javabridge.JClassWrapper(
                'edu.cmu.tetrad.search.Ccd')(indTest)
            ccd.setDepth(depth)
        else:
            algorithm = javabridge.JClassWrapper(
                'edu.cmu.tetrad.algcomparison.algorithm.oracle.pag.Ccd')(indTest)
            ...
            ccd = javabridge.JClassWrapper('edu.pitt.dbmi.algo.bootstrap.GeneralBootstrapTest')(
                tetradData, algorithm, numBootstrap)
            edgeEnsemble = javabridge.get_static_field('edu/pitt/dbmi/algo/bootstrap/BootstrapEdgeEnsemble',
                                                       ensembleMethod,
                                                       'Ledu/pitt/dbmi/algo/bootstrap/BootstrapEdgeEnsemble;')
...
class pc():
    ...
    def __init__(self, df, dataType=0, numCategoriesToDiscretize=4, depth=3, alpha=0.05, verbose=False, priorKnowledge=None, numBootstrap=-1, ensembleMethod='Highest'):
        ...
        if numBootstrap < 1:
            ...
            algorithm = javabridge.JClassWrapper(
                'edu.cmu.tetrad.algcomparison.algorithm.oracle.pattern.Pc')(indTest)
            ...
            pc = javabridge.JClassWrapper('edu.pitt.dbmi.algo.bootstrap.GeneralBootstrapTest')(
                tetradData, algorithm, numBootstrap)
            edgeEnsemble = javabridge.get_static_field('edu/pitt/dbmi/algo/bootstrap/BootstrapEdgeEnsemble',
                                                       ensembleMethod,
                                                       'Ledu/pitt/dbmi/algo/bootstrap/BootstrapEdgeEnsemble;')
...
class pcstablemax():
    ...
    def __init__(self, df, dataType=0, numCategoriesToDiscretize=4, depth=3, maxPathLength=3, useHeuristic=True, alpha=0.05, verbose=False, priorKnowledge=None, numBootstrap=-1, ensembleMethod='Highest'):
        ...
        if numBootstrap < 1:
            pcmax = javabridge.JClassWrapper(
                'edu.cmu.tetrad.search.PcStableMax')(indTest)
            pcmax.setDepth(depth)
            pcmax.setMaxPathLength(maxPathLength)
            pcmax.setUseHeuristic(useHeuristic)
        else:
            algorithm = javabridge.JClassWrapper(
                'edu.cmu.tetrad.algcomparison.algorithm.oracle.pattern.PcStableMax')(indTest)
            ...
            pcmax = javabridge.JClassWrapper('edu.pitt.dbmi.algo.bootstrap.GeneralBootstrapTest')(
                tetradData, algorithm, numBootstrap)
            edgeEnsemble = javabridge.get_static_field('edu/pitt/dbmi/algo/bootstrap/BootstrapEdgeEnsemble',
                                                       ensembleMethod,
                                                       'Ledu/pitt/dbmi/algo/bootstrap/BootstrapEdgeEnsemble;')
...
class pcstable():
    ...
    def __init__(self, df, dataType=0, numCategoriesToDiscretize=4, depth=3, alpha=0.05, verbose=False, priorKnowledge=None, numBootstrap=-1, ensembleMethod='Highest'):
        ...
        if numBootstrap < 1:
            pcstable = javabridge.JClassWrapper(
                'edu.cmu.tetrad.search.PcStable')(indTest)
            pcstable.setDepth(depth)
        else:
            algorithm = javabridge.JClassWrapper(
                'edu.cmu.tetrad.algcomparison.algorithm.oracle.pattern.PcStable')(indTest)
            ...
            pcstable = javabridge.JClassWrapper('edu.pitt.dbmi.algo.bootstrap.GeneralBootstrapTest')(
                tetradData, algorithm, numBootstrap)
            edgeEnsemble = javabridge.get_static_field('edu/pitt/dbmi/algo/bootstrap/BootstrapEdgeEnsemble',
                                                       ensembleMethod,
                                                       'Ledu/pitt/dbmi/algo/bootstrap/BootstrapEdgeEnsemble;')
...
class cpc():
    ...
    def __init__(self, df, dataType=0, numCategoriesToDiscretize=4, depth=3, alpha=0.05, verbose=False, priorKnowledge=None, numBootstrap=-1, ensembleMethod='Highest'):
        ...
        if numBootstrap < 1:
            cpc = javabridge.JClassWrapper(
                'edu.cmu.tetrad.search.Cpc')(indTest)
            cpc.setDepth(depth)
        else:
            algorithm = javabridge.JClassWrapper(
                'edu.cmu.tetrad.algcomparison.algorithm.oracle.pattern.Cpc')(indTest)
            ...
            cpc = javabridge.JClassWrapper('edu.pitt.dbmi.algo.bootstrap.GeneralBootstrapTest')(
                tetradData, algorithm, numBootstrap)
            edgeEnsemble = javabridge.get_static_field('edu/pitt/dbmi/algo/bootstrap/BootstrapEdgeEnsemble',
                                                       ensembleMethod,
                                                       'Ledu/pitt/dbmi/algo/bootstrap/BootstrapEdgeEnsemble;')
...
class cpcstable():
    ...
    def __init__(self, df, dataType=0, numCategoriesToDiscretize=4, depth=3, alpha=0.05, verbose=False, priorKnowledge=None, numBootstrap=-1, ensembleMethod='Highest'):
        ...
        if numBootstrap < 1:
            ...
            algorithm = javabridge.JClassWrapper(
                'edu.cmu.tetrad.algcomparison.algorithm.oracle.pattern.CpcStable')(indTest)
            ...
            cpcstable = javabridge.JClassWrapper('edu.pitt.dbmi.algo.bootstrap.GeneralBootstrapTest')(
                tetradData, algorithm, numBootstrap)
            edgeEnsemble = javabridge.get_static_field('edu/pitt/dbmi/algo/bootstrap/BootstrapEdgeEnsemble',
                                                       ensembleMethod,
                                                       'Ledu/pitt/dbmi/algo/bootstrap/BootstrapEdgeEnsemble;')
...
class bayesEst():
    ...
    def __init__(self, df, depth=3, alpha=0.05, verbose=False, priorKnowledge=None):
        ...
        dags = javabridge.JClassWrapper(
            'edu.cmu.tetrad.search.DagInPatternIterator')(self.tetradGraph)
        dagGraph = dags.next()
...
class randomDag():
    ...
    def __init__(self, seed=None, numNodes=10, numEdges=10):
        ...
        while initEdges < numEdges:
            graph = javabridge.static_call("edu/cmu/tetrad/graph/GraphUtils", "randomGraph",
                                           "(IIIIIIZ)Ledu/cmu/tetrad/graph/Graph;", numNodes, 0, numEdges, 30, 15, 15, False)
